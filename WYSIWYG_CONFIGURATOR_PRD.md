...
- Настраиваемый URL скрипта виджета (для размещения на своём сервере)
- Настройка всех текущих параметров `initWidget`
- Модальное окно с фрагментом кода, копированием и ссылкой на документацию
...
- Переключатель **Desktop** / **Mobile** меняет ширину контейнера превью:
  ...
  - Mobile: ~375px (типичная ширина мобильного viewport)
...
|----------------|-----------|------------|---------------------------|
| **Widget script URL** | `string` | Текстовое поле (URL) | `https://yourinho.github.io/integrations_widget/albato-widget.iife.js`. Позволяет указать свой сервер (например, staging, собственный CDN). |
| `regions`      | `number[]`| Чекбоксы или multi-select | Все (пусто) = показывать все. Опции: 2 (BR), 3 (Global) |
| `font`         | `string`  | Текстовое поле | `""` (Inter по умолчанию). Подсказка: `'Open Sans', sans-serif` |
| `colors`       | `object`  | 7 полей color picker + hex input | См. таблицу цветов ниже |
...
2. В модалке отображается:
   - Фрагмент кода для вставки (HTML + script с вызовом `initWidget` и всеми выбранными параметрами)
   - Кнопка «Скопировать»
...
- Параметры с дефолтными значениями можно опускать (для читаемости) или включать — на усмотрение реализации (рекомендуется опускать для минимального примера)
...
### Адаптивность
- **Конфигуратор (layout):** только desktop (ширина viewport ≥ 1024px). На меньших экранах — сообщение «Используйте устройство с большим экраном» или упрощённый layout (на усмотрение).
- **Превью виджета:** desktop (ширина ~1200px) и mobile (ширина ~375px) — переключатель в UI.

### Производительность
- Обновление превью при вводе — без задержки (debounce не обязателен, т.к. виджет перерисовывается быстро; при необходимости debounce 100–200 ms)
...
| Тема | Решение |
|------|---------|
| Стек | На усмотрение (React, Vue, Svelte или vanilla JS) |
| URL виджета | Настраиваемый параметр в UI. По умолчанию: `https://yourinho.github.io/integrations_widget/albato-widget.iife.js`. Позволяет размещать виджет на своём сервере. |
| Режим обновления превью | В реальном времени при изменении параметров |
| Формат кода в модалке | Только фрагмент (div + script), не полная страница |
